...
@export var floor_node: NodePath
@export var block_layers: Array[NodePath] = []
@export var refresh_every := 0.25
@export var show_empty := false
@export var alpha := 0.35
	...
	for y in range(used.position.y, used.position.y + used.size.y):
		for x in range(used.position.x, used.position.x + used.size.x):
			...
			if blocked:
				...
				col = Color(0, 1, 0, alpha)
			elif show_empty:
				col = Color(0.5, 0.5, 0.5, alpha)
	...
	for n in _blocks:
		if n is TileMapLayer:
			...
			if tm.get_cell_source_id(0, c) != -1:
				...
				if td2 == null:
					return true
				if td2.get_custom_data("Blocked") == true:
					return true
	return false
